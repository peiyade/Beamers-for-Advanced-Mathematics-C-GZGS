...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%/home/peiy/Documents/Teaching/Beamers /home/peiy/Documents/Teaching/教案.pdf
%
...
\begin{frame}{分段函数的连续性}{第二类间断点}
	\begin{columns}[onlytextwidth]
		\column{0.4\textwidth}
		\[
			f(x) = e^{\displaystyle\frac{1}{|x|-1}}.
		\]
		\column{0.6\textwidth}
		\begin{figure}
			\centering
			% Requires \usepackage{graphicx}
			\includegraphics[width=5cm]{(Abs[x]-1)}
				\end{figure}
				\end{columns}
				\end{frame}
				...
				\begin{frame}{连续函数的性质}{初等函数的连续性}
					\begin{block}{基本初等函数的连续性}
						$x^n (x>0,n\in \mathbb{N}^+)$, $\sin(x)$, $\log x$ 是连续的.
					\end{block}
					Why?
					\pause
					\begin{exampleblock}{反函数连续性扩张定义}
						$x^{\frac{1}{n}} (x>0,n\in \mathbb{N}^+)$, $\arcsin(x)$, $e^{x}$, 在其定义域上是连续的.
					\end{exampleblock}
					\pause
				...
				\begin{frame}{连续函数的性质}{连续函数在闭区间上的连续性}
					...
					\end{equation*}
					\pause 以及推论: 闭区间上的连续函数是有界的. \pause (开区间,半开半闭区间则没有这个结论)
				\end{frame}
